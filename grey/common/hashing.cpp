...
namespace grey::common::hashing {
    ...
    std::string md5(const std::string& input) {
        ...
        std::vector<uint8_t> m(input.begin(), input.end());
        m.push_back(0x80);
        while(m.size() % 64 != 56) m.push_back(0);
        uint64_t bits = input.size() * 8;
        for(int i = 0; i < 8; ++i) m.push_back(bits >> (i * 8) & 0xff);
        ...
        uint32_t digest[] = {a0,b0,c0,d0};
        std::ostringstream ss;
        for(auto w : digest)
            for(int i = 0; i < 4; ++i)
                ss << std::hex << std::setw(2) << std::setfill('0') << ((w >> (i * 8)) & 0xff);
        return ss.str();
    }
